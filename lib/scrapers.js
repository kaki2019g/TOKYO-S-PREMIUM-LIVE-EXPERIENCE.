...
const USER_AGENT =
  process.env.USER_AGENT || "JAZZ-THREE/1.0 (+schedule aggregator; contact the site operator)";
...
function parseModernClubMonth(html, config, targetMonth) {
  const $ = cheerio.load(html);
  const events = [];

  $(".m-schedule-list__row").each((_, rowElement) => {
    const row = $(rowElement);
    const days = row
      .find(".m-schedule-list__date .m-schedule-list__date-num")
    ...
    const eventDates = days.map((day) => {
      ...
      previousDay = day;
      return localDateString(eventYear, eventMonth, day);
    });

    row.find("a.c-schedule-list-card[href]").each((__, cardElement) => {
      const card = $(cardElement);
      const title = cleanText(
        (card.find(".c-schedule-list-card__title").html() || "").replace(/<br\s*\/?>/gi, " "),
      );
      if (!title) return;

      const price = cleanText(card.find(".c-schedule-list-card__charge-price").text());
      const url = absoluteUrl(card.attr("href"), config.scheduleBase) || config.venueUrl;
      const image = absoluteUrl(card.find("img").attr("src"), config.scheduleBase);
      ...
      eventDates.forEach((date) => {
        events.push({
          ...
          genre: "",
          shows: [],
          price: price || "公式サイトで確認",
          availability: "公式サイトで確認",
...
function parseLegacyClubMonth(html, config, targetMonth) {
  const $ = cheerio.load(html);
  const events = [];
...
export function parseClubMonth(html, config, targetMonth) {
  const $ = cheerio.load(html);
  if ($(".m-schedule-list__row a.c-schedule-list-card[href]").length) {
    return parseModernClubMonth(html, config, targetMonth);
  }
  return parseLegacyClubMonth(html, config, targetMonth);
}
