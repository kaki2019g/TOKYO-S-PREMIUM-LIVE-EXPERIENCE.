...
`origin` から現在のブランチをリベース形式でプルした後、現在の変更をまとめてコミットし、
プッシュ直前にもう一度最新状態を取り込んでからプッシュします。
...
などは `.gitignore` により対象外です。開始時のプルでは `--autostash` を使用するため、未コミットの
追跡済み変更は一時退避され、プル完了後に復元されます。変更がない場合も未送信コミットをプッシュします。
競合が発生した場合は処理を停止し、強制プッシュは行いません。
